...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM evaluation_answers_faculty_dean 
    ...
    $stmt = $conn->prepare("INSERT INTO evaluation_answers_faculty_dean 
    (evaluation_id, $target_column, evaluator_id, academic_id, question_id, rate, comment) 
    VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    foreach ($question_ids as $question_id) {
        if (isset($ratings[$question_id]) && !empty($ratings[$question_id])) {
            ...
            if (!$stmt->execute([$evaluation_id, $target_id, $evaluator_id, 
            $academic_id, $question_id, $rate, $comment])) {
                $success = false;
            }
